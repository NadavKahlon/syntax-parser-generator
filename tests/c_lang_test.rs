...
fn test_c_ast_pipeline() {
                ...
                statements: vec![
                    ...
                    Box::from(ast::LocalStatement::While {
                        ...
                        body: Box::new(ast::LocalStatement::Block {
                            statements: vec![Box::from(ast::LocalStatement::IfElse {
                                condition: Box::from(ast::Expression::IntLiteral { value: 56 }),
                                true_branch: Box::new(ast::LocalStatement::Block {
                                    statements: vec![Box::from(
                                ...
                                false_branch: Box::new(ast::LocalStatement::Block {
                                    statements: vec![Box::from(ast::LocalStatement::If {
                                        condition: Box::from(ast::Expression::Assignment {
                                            lhs: Box::from(ast::LValue::Variable {
                                                name: "local".to_owned(),
                                            }),
                                            rhs: Box::new(ast::Expression::IntLiteral { value: 8 }),
                                        }),
                                        true_branch: Box::new(ast::LocalStatement::Evaluation {
                                            expression: Box::from(ast::Expression::Assignment {
                                                lhs: Box::from(ast::LValue::Variable {
                                                    name: "local".to_owned(),
                                                }),
    ...
    assert_eq!(
        c_parser.parse(c_lexical_analyzer.analyze(&mut source_reader)),
        Some(expected)
    );
}
