...
type CalculatorTranslator = SyntaxDirectedTranslator<LexemeType, Context, Satellite>;
...
fn build_calculator() -> CalculatorTranslator {
    ...
    builder.register_rule("expression", vec!["INTEGER"], Context::integer_to_expr);
...
fn test_successful_calculator_translator() {
    ...
    assert_eq!(
        calc.translate(
            &mut c,
            vec![Lexeme::new(LexemeType::Integer, "8")].into_iter(),
        ),
        Some(Some(8))
    );
...
fn test_failing_calculator_translator() {
    ...
    // 5 * (1))
    assert_eq!(calc.translate(&mut c, vec![].into_iter()), None,);
}
