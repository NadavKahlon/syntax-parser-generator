    ...
    Tag: OrderlyHandled,
{
    ...
    ) -> Self
    {
        ...
        Self { firsts_for_nonterminals: first_for_nonterminals, rules, rules_for_nonterminals }
    }
    ...
    fn get_produced_firsts(&self, nonterminal: Handle<Nonterminal>)
                           -> HashSet<Option<Handle<Terminal>>>
    {
        self
            ...
            .iter()
            .flat_map(|&rule| {
                self.firsts_for_string(&self.rules[rule].rhs)
            })
            .collect()
    }

    fn firsts_for_grammar_symbol(&self, grammar_symbol: GrammarSymbol<Terminal, Nonterminal>)
                                 -> HashSet<Option<Handle<Terminal>>>
    {
        match grammar_symbol {
            GrammarSymbol::Terminal(terminal) => {
                vec![Some(terminal)].into_iter().collect()
            }
            GrammarSymbol::Nonterminal(nonterminal) => {
                self
                    ...
                    .get(nonterminal)
                    .expect(
                        "Each nonterminal should have a `firsts` entry associated with it"
                    ).clone()
            }
        }
    }

    fn firsts_for_string(&self, string: &Vec<GrammarSymbol<Terminal, Nonterminal>>)
                         -> HashSet<Option<Handle<Terminal>>>
    {
        let mut i = 0;
        ...
        loop {
            match string.get(i) {
                Some(&symbol) => {
                    let sub_firsts
                        = self.firsts_for_grammar_symbol(symbol);
    ...
    pub fn terminal_firsts_for_string(&self, string: &Vec<GrammarSymbol<Terminal, Nonterminal>>)
